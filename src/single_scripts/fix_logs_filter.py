...
def generate_outputs(file_path: str, filters_dict: dict[str, list[tuple[int,str]]]) -> None:
    ...
    with open(file_path, 'r') as file:
        ...
        if "8=FIXT.1.19=" not in lines:
                print(f"LOG FORMAT CORRUPTED. RECOVER SERVICE WILL TRY TO RECOVER TABULATION.")
...
def recover_soh(line: str, marker):
    for tag in FIX_TAGS:
        line = line.replace(f"{marker}{tag}=", f"{tag}=")
    return line
